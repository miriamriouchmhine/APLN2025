...
def generar_resumen_ollama(datos_procesados: str, modelo: str = "gemma3",modo: str = "Comparativa") -> str:
...
""" 
    ...
    response = ollama.generate(
        model= modelo, #"deepseek-r1:14b",#gemma3
        prompt=prompt,
...
def main(ruta_csv: str):
    ...
    modes = ["Comparativa", "ultimoAño"]
    models = ["neural-chat", "granite3.2", "gemma3", "gemma3:12b", "llama3.2", "llama3.1", "deepseek-r1:8b", "deepseek-r1:14b", "phi4", "orca2:13b"]
    ...
    for model in models:    
        ...
        for mode in modes:
            initimer = time.time()
            if mode == "Comparativa":
                informe = generar_resumen_ollama(datos, model, mode)
                print("=== COMPARACIÓN===")
            else:
                print("===INFORME 2025-2026===")
                informe = generar_resumen_ollama(datos, model, mode)

            fintimer = time.time()
            print(f"Tiempo de ejecución: {fintimer - initimer:.2f} segundos")

            ficheroSalida = f"Informe_{model}_{mode}.txt"
            with open(f"./output/{ficheroSalida}", "w", encoding="utf-8") as f:
                f.write(informe)
            print(f"Informe guardado como {ficheroSalida}")        


main("./dataIn/becas_structurado.csv")
